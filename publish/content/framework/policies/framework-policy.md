...
# <ORG_NAME> AI Agent Governance Policy

**Document:** AI Agent Governance Policy
...
1. A formal exception request file has been created in the appropriate subfolder under `policies/exceptions/`. **(POL-017)**
...
Any approved reassignment must document the reassignment reason, date, and approving authority in the approved C02 exception PR (`policies/exceptions/policy/`); the change is then reflected by GitHub Project access and the anchor issue's assignees. There is no per-project state file to edit. **(POL-054)**
...
### 7.0 Always-rules — the C01 digest every agent carries

These are the clauses that must hold on **every turn** of an agent session, not merely at its
start. They are marked because the harness renderer copies this block verbatim into every
agent's context file (`AGENTS.md`, `CLAUDE.md`, `.cursor/rules/agent.mdc`, …), so an agent has
them in context without having to choose to open a second file.

**This block is the single source; the copies are generated.** Edit it here and re-render — the
copies carry a do-not-edit banner. A reference would have been tidier and would have left the
rules one unopened file away, which is the difference between governance an agent has and
governance it could have looked up.
...
### 7.0.1 What gov guarantees about an agent's context

This is the guarantee the framework makes to the organization, and the one an adopter is
entitled to rely on. It is stated as what gov *does*, not as what agents *should*.

**gov guarantees that the governance requirements are in the agent's context at launch and on
...
governed session that silently began ungoverned is the failure this clause exists to prevent, so
the refusal is the correct outcome and names the path it checked. **(POL-429)**

**One mechanism, for every approved agent.** No agent is governed by a means another agent
lacks. A vendor-specific hook — even a working one — would make that vendor the better-governed
choice for a reason unrelated to its merits, and would bias the selection the organization makes
when it approves agents. Consistency across the approved list takes precedence over any marginal
per-vendor capability. **(POL-430)**

**Changes in governance are picked up in new sessions**, because gov re-derives the organization's
ratified governance from the default branch (POL-086a) and re-places each agent's file at the
start of every session.

**Mid-session, the developer can force the pickup.** `gov sync` merges the current ratified
governance into the project and re-places every agent's context file, so everything changed up
to that moment is on disk. What gov cannot do is reach into a session already running: whether a
live agent re-reads its instructions from disk is the agent's own behaviour, not gov's. So `gov
sync` also prints the one sentence to send into the running session, and it is the same sentence
...
The list of approved LLM providers and models is maintained by the Infrastructure Owner in `org-config.yaml (authorized_agents)`. This list is the authoritative source for what is and is not permitted. **(POL-136)**
...
Using a prohibited LLM provider — one not listed in `org-config.yaml (authorized_agents)` — is a C01 violation **(POL-138)**. The agent must hard stop and surface this to the human immediately.
...
1. The requester creates an exception request file in the appropriate exceptions subfolder: **(POL-152)**
   - Legal exceptions → `policies/exceptions/legal/`
   - Infrastructure exceptions → `policies/exceptions/infrastructure/`
   - Architecture exceptions → `policies/exceptions/architecture/`
   - Policy exceptions → `policies/exceptions/policy/`
...
`framework/policies/roles.md` defined the same roles this policy already defines, and `POL-402`
...
POL-136: Approved LLM providers and models are listed in org-config.yaml (authorized_agents), maintained by Infrastructure Owner.
...
POL-152: C02 exception requests must be filed in the appropriate exceptions subfolder under policies/exceptions/.
