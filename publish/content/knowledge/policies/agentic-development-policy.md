...
The following `gov` subcommands constitute the authorized tooling for project and knowledge lifecycle management. `gov` is the org's CLI (npm package `@svayam-opensource/gov-work`, Node 24). Agents and developers must use these commands rather than performing equivalent operations manually. **(POL-164)**
