...
3. Clone repo into `<agent_work_root>/PRJ-<board#>-<slug>/<repo-name>/`
4. Create branch `BRNCH-<board#>-<slug>` from `base_branch`
...
6. Add entry to `repos[]` in `project.yaml`:
   ...
   ```
7. Commit updated `project.yaml` to `BRNCH-<board#>-<slug>` branch in `<WORKSPACE_REPO>`
...
- Branch `BRNCH-<board#>-<slug>` created in repo and pushed
