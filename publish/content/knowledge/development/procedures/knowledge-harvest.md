...
into durable org knowledge. It is the **quality lever**: a script can only check
...
never by the close script, which checks presence/structure only.

**Output:** `projects/<PRJ>/knowledge/knowledge-close.md` (the manifest, template
below) + the actual proposed org-knowledge changes (via `propose-knowledge`).
`close-project`'s gate refuses to close until the manifest exists and every
...
7. **PRODUCE** — raise the org-knowledge changes via `propose-knowledge` (one or
