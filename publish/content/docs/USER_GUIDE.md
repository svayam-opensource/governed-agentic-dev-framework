...
The `gov` CLI is installed from npm — `npm i -g @svayam-opensource/gov-work` (requires Node 24). It is **not** vendored into repos, so repos carry only data instead of a copy of the framework.
