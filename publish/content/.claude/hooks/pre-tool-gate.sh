...
printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Complete /session-start first: run the session-start protocol and post the context manifest (framework/CLAUDE.md §0) before changing code. Mutating tools unlock once /session-start acknowledges this session."}}'
exit 0
