...
export function parseOriginOwnerRepo(url) {
 ...
 */
export function deriveOrgConfig(answers, ctx) {
    const e = ctx.existing ?? {};
    const pick = (k, fallback) => answers[k] ?? e[k] ?? fallback;
...
export function renderOrgConfig(v) {
...
export function readExistingOrgConfig(text) {
    const scalar = (key) => {
