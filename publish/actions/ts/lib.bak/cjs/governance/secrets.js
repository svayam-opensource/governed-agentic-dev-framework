...
const PLACEHOLDER_RE = /^(?:|\s*|x+|\.\.\.|-+|\*+|changeme|example|placeholder|your[-_].*|my[-_].*|some[-_].*|redacted|dummy|sample|test(?:ing)?|fake|none|null|nil|true|false|\$\{[^}]+\}|\{\{[^}]+\}\}|\$[A-Za-z_][A-Za-z0-9_]*|<[^>]+>)$/i;
...
const ASSIGNMENT_RE = /\b(?:password|passwd|pwd|api[_-]?key|secret[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret)\b\s*[:=]\s*(["'])([^"']{6,})\1/i;
    ...
    for (const rel of ctx.files ?? []) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const [label, pat] of PATTERNS) {
                if (pat.test(line))
                    errors.push(`${rel}:${i + 1}: ${label}: ${snippet}`);
            }
            const m = ASSIGNMENT_RE.exec(line);
            if (m && !isPlaceholder(m[2]))
                errors.push(`${rel}:${i + 1}: hardcoded credential: ${snippet}`);
        }
    }
