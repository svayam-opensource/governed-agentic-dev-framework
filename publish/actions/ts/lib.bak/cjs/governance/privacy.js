...
export function privateValuesFromOrgConfig(mainConfigText) {
    const out = [];
...
function scannable(rel) {
    ...
    if (parts.some((p) => p.startsWith(".")))
        return false; // skip dotfiles/dirs
...
export function makePrivacyValidator(mainConfigText) {
    const values = privateValuesFromOrgConfig(mainConfigText);
    return (ctx) => {
        const errors = [];
        if (values.length === 0)
            return { name: "privacy", ok: true, errors };
        for (const rel of ctx.files ?? []) {
            ...
            for (const { key, value } of values) {
                if (ATTRIBUTION_KEYS.has(key) && ATTRIBUTION_FILES.has(name))
                    continue; // legit attribution
                for (let i = 0; i < lines.length; i++) {
                    if (lines[i].includes(value))
                        errors.push(`${rel}:${i + 1}: leak of ${key}='${value}'`);
                }
            }
        }
