 ...
 */
function confirmHome(env, home, org) {
    const cfg = env.govConfigAt(home);
    if (cfg === null)
        return { why: "not-a-gov-repo" };
    if (cfg.org !== org)
        return { why: "org-mismatch", found: cfg.org };
...
export function resolveFailureMessage(r) {
    switch (r.reason) {
        case "no-active-org":
            return "No active org is set. Run `gov-work org use <github_org>` to choose one.";
        case "org-conflict":
            return (`You're in ${r.cwdOrg}'s workspace but the active org is ${r.activeOrg}. ` +
                `Run \`gov-work org use ${r.cwdOrg}\` (or cd into a ${r.activeOrg} workspace).`);
        case "no-home":
            return (`Active org ${r.activeOrg} has no registered gov home. ` +
        ...
        case "pointer-mismatch": {
            ...
            const tail = r.detail.why === "not-a-gov-repo"
