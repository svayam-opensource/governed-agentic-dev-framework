...
import { createGhBoard } from "../lifecycle/gh-board.js";
...
import { route, routeOrg } from "./dispatch.js";
 ...
 */
export async function runSetupCommand(argv, now = new Date().toISOString()) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (raw) {
            try {
                ...
                if (pkg.name === "@svayam-opensource/gov-work" && pkg.version)
                    return pkg.version;
            }
            catch {
                /* keep walking */
...
export async function gatherMenuContext() {
    const fs = createNodeFs();
 ...
 */
export async function runCredsCommand(argv) {
    const fs = createNodeFs();
    const resolve = prjResolveGov(createNodeEnv());
    if (!resolve.ok) {
        process.stderr.write("gov-work creds: no gov workspace resolved — run `gov-work onboard`/`gov-work setup` first.\n");
        return 1;
    }
    const cfgText = fs.readFile(path.join(resolve.home, "org-config.yaml"));
    if (!cfgText) {
        process.stderr.write("gov-work creds: org-config.yaml not found.\n");
        return 1;
    }
...
export function runAny(argv) {
    if (argv[0] === "setup")
        return runSetupCommand(argv);
    if (argv[0] === "creds")
        return runCredsCommand(argv);
    return main(argv);
}
/** The command reference shown under the Help menu (grouped) / per-command. */
export function helpLines(command) {
    if (command)
        return ["", `  gov-work ${command} — run \`gov-work ${command} --help\`, or see the README command reference.`, ""];
    const groups = {
    ...
    for (const [g, cmds] of Object.entries(groups))
        out.push(`    ${g.padEnd(20)} ${cmds.join(" · ")}`);
    out.push("");
    return out;
}
    ...
    if (resolved.ok) {
        ...
        if (cfgText) {
            ...
            workDeps = {
                ...
                me: ctx.user ?? null,
                canWriteBoard: (n) => tryRun("gh", ["api", "graphql", "-f", "query=query($o:String!,$n:Int!){organization(login:$o){projectV2(number:$n){viewerCanUpdate}}}", "-F", `o=${config.githubOrg}`, "-F", `n=${n}`, "--jq", ".data.organization.projectV2.viewerCanUpdate"]) !== "false",
                run: runAny,
                prompt: async () => "",
                print: () => { },
            };
...
export function main(argv, now = new Date().toISOString()) {
    ...
    if (parsed.command === "deps") {
        ...
        for (const line of formatDepsReport(report))
    ...
    if (parsed.command === "publish") {
        ...
        for (const line of formatPublishGate(gate))
            process.stdout.write(`${line}\n`);
        return gate.ok ? 0 : 1;
    }
    ...
    if (parsed.command === "upgrade") {
        ...
        if (contentSync) {
            ...
            if (from) {
                ...
                try {
                    ...
                    process.stderr.write(`gov-work upgrade: ${e.message}\n`);
                    return 1;
                }
            }
            try {
        ...
        try {
            ...
            if (pkg)
                cliVersion = JSON.parse(pkg).version ?? "0.0.0";
        }
        catch {
            /* keep default */
        }
    ...
    if (parsed.command === "doctor") {
        ...
        try {
            const pkg = fs.readFile(fileURLToPath(new URL("../../../package.json", import.meta.url)));
            if (pkg)
                cliVersion = JSON.parse(pkg).version ?? "unknown";
        }
        catch {
            /* leave "unknown" */
        }
    ...
    if (parsed.command === "org") {
        ...
        for (const line of orgResult.lines)
            process.stdout.write(`${line}\n`);
    ...
    if (!("GOV_SKIP_PREFLIGHT" in process.env)) {
        ...
        if (!pf.ok) {
            for (const line of renderGap(pf.gap))
