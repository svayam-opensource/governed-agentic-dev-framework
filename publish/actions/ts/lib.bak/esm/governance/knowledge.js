...
export function checkKnowledge(ctx) {
    const errors = [];
    const abs = (rel) => path.join(ctx.repoRoot, rel);
    if (!ctx.fs.pathExists(abs("knowledge"))) {
        // The framework/template SOURCE repo has no instantiated org tree — nothing to check.
        if (ctx.fs.pathExists(abs("framework")))
            return { name: "knowledge", ok: true, errors: [] };
    ...
    for (const rel of docs) {
        ...
        for (const [key, allowed] of [["domain", DOMAINS], ["layer", LAYERS], ["compliance", COMPLIANCE], ["status", STATUSES]]) {
            if (!allowed.has(fm[key] ?? ""))
                errors.push(`${rel}: front-matter ${key}='${fm[key]}' invalid (POL-408)`);
        }
        if (!fm.owner)
            errors.push(`${rel}: front-matter owner missing (POL-408)`);
        ...
        if (parts[1] === "paths" && base !== "README.md") {
            if (text.includes("```"))
                errors.push(`${rel}: journey docs are links-only — code block found (POL-410)`);
            if (IMG_RE_TEST.test(text))
                errors.push(`${rel}: journey docs are links-only — image found (POL-410)`);
            if ([...text.matchAll(LINK_RE_G)].length < 3)
                errors.push(`${rel}: journey doc has fewer than 3 links — is it a journey? (POL-410)`);
        }
    }
