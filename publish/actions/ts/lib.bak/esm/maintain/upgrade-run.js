...
/**
 * `gov-work upgrade --from <content>` runner — the fs-backed shell around the pure
...
export function runUpgradeSync(contentDir, adopterDir, opts) {
    const manifestPath = path.join(contentDir, "MANIFEST.yaml");
    if (!fs.existsSync(manifestPath))
        return { code: 1, lines: [`gov-work upgrade: no MANIFEST.yaml under ${contentDir}`] };
    ...
    const readAdopter = (rel) => {
        const p = path.join(adopterDir, rel);
        return fs.existsSync(p) && fs.statSync(p).isFile() ? fs.readFileSync(p, "utf8") : null;
    };
    const plan = planUpgrade(entries, { readContent, readAdopter, adopterPaths: () => walk(adopterDir) });
    if (!opts.apply) {
        return { code: 0, lines: ["gov-work upgrade — DRY RUN (no changes written):", "", ...formatPlan(plan), "", "Re-run with --apply to write these changes."] };
    }
    ...
    return {
        code: 0,
        lines: [
            `gov-work upgrade — applied ${res.applied.length} change(s)${res.skipped.length ? `, skipped ${res.skipped.length} conflict(s) for review:` : "."}`,
...
function upgradePrBody(version, appliedCount, plan) {
    const of = (k) => plan.actions.filter((a) => a.kind === k).map((a) => a.dst);
    const conflicts = of("conflict");
    const retired = of("retire");
    return [
        `Syncs this gov workspace to framework content **${version}** — generated by \`gov-work upgrade\`.`,
        ``,
...
export function runUpgradePr(contentDir, adopterDir, opts = {}) {
    if (!fs.existsSync(path.join(contentDir, "MANIFEST.yaml")))
        return { code: 1, lines: [`gov-work upgrade: no MANIFEST.yaml under ${contentDir}`] };
    try {
        git(adopterDir, ["rev-parse", "--git-dir"]);
    }
    catch {
        return { code: 1, lines: ["gov-work upgrade --pr: not a git repository (or no remote). Use --apply for an in-place migration instead."] };
    }
    if (git(adopterDir, ["status", "--porcelain"]))
        return { code: 1, lines: ["gov-work upgrade --pr: working tree has uncommitted changes — commit or stash first."] };
    ...
    const entries = expandEntries(manifest, walk(contentDir));
    const readContent = (rel) => { const p = path.join(contentDir, rel); return fs.existsSync(p) && fs.statSync(p).isFile() ? fs.readFileSync(p, "utf8") : null; };
    const readAdopter = (rel) => { const p = path.join(adopterDir, rel); return fs.existsSync(p) && fs.statSync(p).isFile() ? fs.readFileSync(p, "utf8") : null; };
    const plan = planUpgrade(entries, { readContent, readAdopter, adopterPaths: () => walk(adopterDir) });
    if (plan.actions.every((a) => a.kind === "same"))
        return { code: 0, lines: ["gov-work upgrade: workspace already matches content — nothing to do."] };
    try {
        git(adopterDir, ["checkout", "-b", branch]);
    }
    catch {
        return { code: 1, lines: [`gov-work upgrade --pr: branch '${branch}' already exists — delete it or pass --branch <name>.`] };
    }
    const res = applyUpgrade(plan, {
    ...
    git(adopterDir, ["commit", "-m", `gov-work upgrade: sync framework content to ${version}`]);
    try {
        git(adopterDir, ["push", "-u", "origin", branch]);
    }
    catch (e) {
        return { code: 1, lines: [`Applied on ${branch} but push failed: ${e.message.split("\n")[0]}`] };
    }
    let prUrl;
    try {
        prUrl = execFileSync("gh", ["pr", "create", "--base", base, "--head", branch, "--title", `gov-work upgrade → framework content ${version}`, "--body", upgradePrBody(version, res.applied.length, plan)], { cwd: adopterDir, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }).trim();
    }
    catch (e) {
        return { code: 0, lines: [`Pushed ${branch} (open the PR manually — gh failed): ${e.message.split("\n")[0]}`] };
    }
 ...
 */
export function fetchTemplateContent(templateUrl, ref) {
    ...
    try {
        ...
        cleanup();
        throw new Error(`could not fetch content from ${templateUrl}@${ref}: ${e.message.split("\n").pop()}`, { cause: e });
    }
