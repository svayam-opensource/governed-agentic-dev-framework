...
export function archiveBranch(vcs, repoDir, branch, remote) {
...
export function merge(deps, config, input) {
    const log = deps.log ?? (() => { });
    ...
    if (parsed) {
        ...
        issueUrls = numbers.map((n) => deps.issues.resolveIssueUrl(ref, n)).filter((u) => !!u);
    }
    else {
    ...
    for (const dir of reposMerged)
        archiveBranch(deps.vcs, dir, taskId, remote);
