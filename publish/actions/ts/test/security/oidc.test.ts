...
import { claimsOf, loginClientCredentials } from "../../src/security/oidc.js";
...
describe("oidc — loginClientCredentials (headless service login)", () => {
  it("POSTs client_credentials to the discovered token endpoint and returns the svayam_jwt", async () => {
    ...
    globalThis.fetch = (async (url: string, init?: { body?: unknown }) => {
      ...
      return { ok: true, json: async () => ({ access_token: jwt({ sub: "ip-gov-ci", account_ctx: 1000, roles: ["GOV_CI"], aud: "gov" }), expires_in: 300 }) } as Response;
    }) as typeof fetch;
    try {
      const tokens = await loginClientCredentials({ issuer: "https://iam", clientId: "gov-ci", audience: "gov", scopes: "openid email profile" }, "app-password");
      const tokenCall = calls.find((c) => c.url === "https://iam/token");
      expect(tokenCall, "token endpoint called").to.exist;
      expect(tokenCall!.body).to.contain("grant_type=client_credentials");
      expect(tokenCall!.body).to.contain("client_id=gov-ci");
      expect(tokenCall!.body).to.contain("client_secret=app-password");
