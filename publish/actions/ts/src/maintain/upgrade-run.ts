...
export function runUpgradeSync(contentDir: string, adopterDir: string, opts: { apply: boolean }): UpgradeSyncResult {
  ...
  if (!fs.existsSync(manifestPath)) return { code: 1, lines: [`gov-work upgrade: no MANIFEST.yaml under ${contentDir}`] };
...
export function runUpgradePr(contentDir: string, adopterDir: string, opts: { branch?: string } = {}): UpgradeSyncResult {
  if (!fs.existsSync(path.join(contentDir, "MANIFEST.yaml"))) return { code: 1, lines: [`gov-work upgrade: no MANIFEST.yaml under ${contentDir}`] };
  try { git(adopterDir, ["rev-parse", "--git-dir"]); } catch { return { code: 1, lines: ["gov-work upgrade --pr: not a git repository (or no remote). Use --apply for an in-place migration instead."] }; }
  if (git(adopterDir, ["status", "--porcelain"])) return { code: 1, lines: ["gov-work upgrade --pr: working tree has uncommitted changes — commit or stash first."] };
  ...
  const plan = planUpgrade(entries, { readContent, readAdopter, adopterPaths: () => walk(adopterDir) });
  if (plan.actions.every((a) => a.kind === "same")) return { code: 0, lines: ["gov-work upgrade: workspace already matches content — nothing to do."] };

  try { git(adopterDir, ["checkout", "-b", branch]); } catch { return { code: 1, lines: [`gov-work upgrade --pr: branch '${branch}' already exists — delete it or pass --branch <name>.`] }; }
  const res = applyUpgrade(plan, {
