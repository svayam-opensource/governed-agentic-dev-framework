...
export function credentialsPath(agentWorkRoot: string, identity: string): string {
...
export function credKey(kind: string, scope: string): string {
  return `${kind}:${scope}`;
}
...
export function getCredential(file: string, key: string): string | undefined {
  let text: string;
...
export function listCredentialKeys(file: string): string[] {
  try { return [...parseCredentials(fs.readFileSync(file, "utf8")).keys()]; } catch { return []; }
}
 ...
 */
export function setCredential(file: string, key: string, value: string): void {
...
export function listIdentities(agentWorkRoot: string): string[] {
...
export function identityExists(agentWorkRoot: string, identity: string): boolean {
  try { return fs.statSync(credentialsPath(agentWorkRoot, identity)).isFile(); } catch { return false; }
}

/** The default identity for the prompt — the logged-in user id. Overridable by the user. */
export function defaultIdentity(env: NodeJS.ProcessEnv = process.env): string {
