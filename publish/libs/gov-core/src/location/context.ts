...
/** What `detectContext` needs to know about this machine. All resolved by the CALLER, because how a client
 *  finds its governance home is the client's business — the rule for turning those facts into a mode is not. */
export interface ContextFacts {
  /** the directory the command was invoked from. */
  readonly cwd: string;
  /** the governance repo home, if this machine has one resolved. */
  readonly govHome?: string;
  /** `agent_work_root` from org-config, TILDE ALREADY EXPANDED — the parent of every project dir. */
  readonly agentWorkRoot?: string;
  /** path separator; injectable so a Windows path can be tested from anywhere. */
  readonly sep?: string;
}

/**
 * WHICH CONTEXT AM I IN — the one rule, in one place.
 *
 * `project` when the cwd sits inside a project directory under `agent_work_root`; `governed` when a
 * governance home resolved but the cwd is elsewhere; `none` when neither. The distinction decides which
 * BRANCH a client reads and writes — project branch versus `main` — so two clients disagreeing about it
 * means two clients acting on different repositories while reporting the same thing.
 *
 * It was previously computed inline in the gov-work menu and reimplemented in the gov-cicd plugin: the
 * classic two-copies-of-one-rule, with nothing comparing them.
 */
export function detectContext(facts: ContextFacts): { mode: ContextMode; projectPath?: string } {
  const sep = facts.sep ?? "/";
  const root = facts.agentWorkRoot;
  if (root && facts.cwd.startsWith(root.endsWith(sep) ? root : root + sep)) {
    // the first segment BELOW agent_work_root names the project; deeper paths stay in that project
    const rel = facts.cwd.slice(root.endsWith(sep) ? root.length : root.length + 1);
    const seg = rel.split(sep).filter(Boolean)[0];
    if (seg) return { mode: "project", projectPath: seg };
  }
  return facts.govHome ? { mode: "governed" } : { mode: "none" };
}
