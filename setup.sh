...
ADF_ACTIVATED=false
# Only touch the login shell rc for a REAL interactive setup (stdin is a TTY). Tests/CI pipe
# or redirect stdin, so they skip automatically and never write a developer's rc;
# SETUP_FORCE_SHELL_RC=1 forces it (the rc test), SETUP_SKIP_SHELL_RC=1 disables it.
if [[ "${SETUP_SKIP_SHELL_RC:-}" != "1" ]] && { [[ -t 0 ]] || [[ "${SETUP_FORCE_SHELL_RC:-}" == "1" ]]; }; then
