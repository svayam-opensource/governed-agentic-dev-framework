...
|---|---|---|
| `project_id` | Yes | e.g., `PRJ-26-invoice-api` (passed from close-project) |
...
- `projects/PRJ-<board#>-<slug>/knowledge/` must contain content to synthesize

---

## Steps

1. Read all content from `projects/PRJ-<board#>-<slug>/knowledge/` holistically
...
5. Checkout new branch `BRNCH-<board#>-<slug>-knowledge` from `<DEFAULT_BRANCH>` of `<WORKSPACE_REPO>`
6. Apply proposed changes to `knowledge/` on that branch
7. Commit changes
8. Raise PR: `BRNCH-<board#>-<slug>-knowledge` → `<DEFAULT_BRANCH>`
   - PR title: `[Knowledge Close] PRJ-<board#>-<slug>`
...
- Branch `BRNCH-<board#>-<slug>-knowledge` created with proposed org knowledge changes
...
| Outcome | Action |
|---|---|
| **Merged** | Archive tag `archive/BRNCH-<board#>-<slug>-knowledge` + delete branch; `knowledge_status: merged` |
