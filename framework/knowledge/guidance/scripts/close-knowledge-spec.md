...
|---|---|---|
| `project_id` | Yes | e.g., `PRJ-007-invoice-api` (passed from close-project) |
...
- `projects/PRJ-NNN-<slug>/knowledge/` must contain content to synthesize

---

## Steps

1. Read all content from `projects/PRJ-NNN-<slug>/knowledge/` holistically
...
5. Checkout new branch `brnch-NNN-<slug>-knowledge` from `<DEFAULT_BRANCH>` of `<WORKSPACE_REPO>`
6. Apply proposed changes to `knowledge/` on that branch
7. Commit changes
8. Raise PR: `brnch-NNN-<slug>-knowledge` → `<DEFAULT_BRANCH>`
   - PR title: `[Knowledge Close] PRJ-NNN-<slug>`
...
- Branch `brnch-NNN-<slug>-knowledge` created with proposed org knowledge changes
...
| Outcome | Action |
|---|---|
| **Merged** | Archive tag `archive/brnch-NNN-<slug>-knowledge` + delete branch; `knowledge_status: merged` |
