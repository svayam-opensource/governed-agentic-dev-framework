...
# ── Downgrade guard: never apply an OLDER framework over a newer install ───────
if [[ -n "$PREV_VERSION" ]]; then
  _new="${NEW_VERSION%%-*}"; _prev="${PREV_VERSION%%-*}"
  _lo="$(printf '%s\n%s\n' "$_new" "$_prev" | sort -V | head -n1)"
  if [[ "$_lo" == "$_new" && "$_new" != "$_prev" ]]; then
    if $ALLOW_DOWNGRADE; then
      log_info "DOWNGRADE OVERRIDE: applying framework v$NEW_VERSION over installed v$PREV_VERSION (--allow-downgrade)."
    else
      hard_stop "Refusing setup: framework v$NEW_VERSION is LOWER than installed v$PREV_VERSION.
This would overwrite a newer framework with an older one — stopped for a careful look.
Re-run with --allow-downgrade only if this is genuinely intended."
    fi
  fi
fi
